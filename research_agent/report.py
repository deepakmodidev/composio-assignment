...
<body class="min-h-screen text-slate-700 antialiased">
  ...
  <main id="top" class="mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 lg:px-8">
    <header class="relative overflow-hidden rounded-[2rem] border border-indigo-100 bg-white/90 px-10 py-12 shadow-xl shadow-indigo-100/60">
      ...
      <div class="relative grid items-center gap-12 lg:grid-cols-[1.35fr_.65fr]">
        <div>
          ...
          <h1 id="headline" class="mt-6 max-w-4xl text-5xl font-semibold tracking-[-0.035em] text-slate-950"></h1>
          <p id="subtitle" class="mt-5 max-w-3xl text-lg leading-7 text-slate-600"></p>
          <div id="hero-facts" class="mt-8 flex flex-wrap gap-2"></div>
        </div>
        <aside class="rounded-3xl border border-indigo-100 bg-white/80 p-6 shadow-lg shadow-indigo-100/50">
          <div class="flex items-center justify-between"><div><p class="text-xs font-semibold tracking-[0.16em] text-indigo-700">BUILDABILITY MAP</p><p class="mt-1 text-sm text-slate-500">Corrected portfolio verdicts</p></div><span class="h-2.5 w-2.5 rounded-full bg-emerald-400"></span></div>
          <div class="mt-6 flex items-center gap-6">
            <div id="hero-donut" class="grid h-36 w-36 shrink-0 place-items-center rounded-full"><div class="grid h-24 w-24 place-items-center rounded-full bg-white text-center shadow-inner"><div><p id="hero-ready" class="text-3xl font-semibold tracking-tight text-slate-950"></p><p class="text-xs text-slate-500">ready</p></div></div></div>
            <div class="min-w-0 flex-1 space-y-3 text-sm"><div class="flex items-center justify-between"><span class="flex items-center gap-2"><i class="h-2.5 w-2.5 rounded-full bg-indigo-500"></i>Ready</span><span id="hero-ready-side" class="font-semibold text-slate-900"></span></div><div class="flex items-center justify-between"><span class="flex items-center gap-2"><i class="h-2.5 w-2.5 rounded-full bg-amber-400"></i>Conditional</span><span id="hero-conditional" class="font-semibold text-slate-900"></span></div><div class="flex items-center justify-between"><span class="flex items-center gap-2"><i class="h-2.5 w-2.5 rounded-full bg-slate-300"></i>Unknown</span><span id="hero-unknown" class="font-semibold text-slate-900"></span></div></div>
          </div>
          <p class="mt-5 border-t border-slate-100 pt-4 text-xs leading-5 text-slate-500">Verdicts are evidence classifications, not a claim that credentials were tested.</p>
        </aside>
      </div>
    </header>

    <section aria-label="Computed metrics" class="mt-8 grid gap-4 lg:grid-cols-5" id="metric-cards"></section>
    ...
    <section id="proof" class="mt-14 rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-indigo-50 via-white to-cyan-50 p-8 shadow-lg shadow-indigo-100/40">
      <div class="flex items-end justify-between gap-8"><div><p class="text-xs font-semibold tracking-[0.18em] text-indigo-700">HOW TRUST WAS BUILT</p><h2 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">One loop, three independent checks</h2></div><p class="max-w-xl text-sm leading-6 text-slate-600">Search discovered candidates; only directly fetched passages became evidence. Browser outcomes are non-human and are reported separately from the applicant audit.</p></div>
      <div id="workflow" class="mt-7 grid gap-3 md:grid-cols-4"></div>
      <div class="mt-6 grid gap-4 lg:grid-cols-2"><div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 class="font-semibold text-slate-950">Rendered-page holdout</h3><div id="browser-proof" class="mt-3 text-sm leading-6 text-slate-600"></div></div><div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 class="font-semibold text-slate-950">Measured accuracy</h3><div id="accuracy" class="mt-3 space-y-2 text-sm leading-6 text-slate-600"></div></div></div>
    </section>
    ...
    <section id="dataset" class="mt-14 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div class="mt-5 flex items-center justify-between border-t border-slate-100 pt-4"><p id="table-count" class="text-sm text-slate-500"></p><div class="flex items-center gap-2"><button id="page-prev" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">Previous</button><span id="page-label" class="min-w-24 text-center text-xs text-slate-500"></span><button id="page-next" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">Next</button></div></div>
    </section>

    <footer class="mt-10 rounded-3xl border border-slate-200 bg-white/80 p-6 text-sm leading-6 text-slate-500"><div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p class="font-medium text-slate-800">Reproduce locally</p><p class="mt-1"><code class="rounded bg-slate-100 px-2 py-1 text-xs text-slate-700">uv run python -m research_agent.report --prepare-review</code> → complete the fixed CSV → <code class="rounded bg-slate-100 px-2 py-1 text-xs text-slate-700">uv run python -m research_agent.report --final</code></p></div><div class="flex gap-3"><a class="font-medium text-indigo-700 hover:text-indigo-900" href="https://github.com/deepakmodidev/composio-assignment" target="_blank" rel="noreferrer">Source repo ↗</a><a class="font-medium text-indigo-700 hover:text-indigo-900" href="https://deepakmodidev.github.io/composio-assignment/" target="_blank" rel="noreferrer">Live page ↗</a></div></div><p class="mt-3 text-xs">Tailwind’s browser CDN is the only UI dependency, so styling requires network access.</p></footer>
  </main>
  <script>
  ...
  const buildability = metrics.distributions.buildability; const ready = buildability.ready || 0; const conditional = buildability.conditional || 0; const unknown = buildability.unknown || 0; const readyStop = ready / metrics.app_count * 100; const conditionalStop = (ready + conditional) / metrics.app_count * 100;
  document.getElementById('hero-donut').style.background = `conic-gradient(#6366f1 0 ${{readyStop}}%, #fbbf24 ${{readyStop}}% ${{conditionalStop}}%, #cbd5e1 ${{conditionalStop}}% 100%)`;
  document.getElementById('hero-ready').textContent = ready; document.getElementById('hero-ready-side').textContent = ready; document.getElementById('hero-conditional').textContent = conditional; document.getElementById('hero-unknown').textContent = unknown;
  ...
  const cardAccents = ['border-t-indigo-500', 'border-t-violet-500', 'border-t-cyan-500', 'border-t-emerald-500', 'border-t-amber-500']; const cardRoot = document.getElementById('metric-cards'); cards.forEach(([name, value, note], index) => {{ const card = document.createElement('article'); card.className = `rounded-2xl border border-t-4 border-slate-200 ${{cardAccents[index]}} bg-white p-5 shadow-md shadow-slate-200/40`; add(card, 'p', 'text-xs font-medium tracking-wide text-slate-500', name.toUpperCase()); add(card, 'p', 'mt-2 text-2xl font-semibold tracking-tight text-slate-950', String(value)); add(card, 'p', 'mt-1 text-xs text-slate-500', note); cardRoot.append(card); }});
  ...
  const table = document.getElementById('app-table'); const pageSize = 12; let currentPage = 1;
  function renderTable() {{ const query = document.getElementById('search').value.trim().toLowerCase(); const category = categoryFilter.value; const filtered = state.records.filter(row => (!category || row.category === category) && JSON.stringify(row).toLowerCase().includes(query)); const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize)); currentPage = Math.min(currentPage, pageCount); const visible = filtered.slice((currentPage - 1) * pageSize, currentPage * pageSize); table.replaceChildren(); visible.forEach(row => {{ const tr = document.createElement('tr'); tr.className = 'border-b border-slate-100 hover:bg-indigo-50/30'; [[`${{row.id}}. ${{row.name}}`, row.description], [row.category], [row.auth], [row.access], [row.api_breadth, row.api_style], [row.mcp], [row.buildability]].forEach((values,index) => {{ const td = document.createElement('td'); td.className = 'px-3 py-4 align-top text-slate-600'; add(td, 'div', index===0 ? 'font-semibold text-slate-950' : 'font-medium text-slate-800', values[0]); if (values[1]) add(td, 'p', 'mt-1 max-w-sm text-xs leading-5 text-slate-500', values[1]); tr.append(td); }}); const evidenceCell = document.createElement('td'); evidenceCell.className = 'px-3 py-4 align-top'; Object.entries(row.evidence).filter(([, url]) => url).forEach(([field, url]) => {{ const link = document.createElement('a'); link.href = url; link.target = '_blank'; link.rel = 'noreferrer'; link.className = 'mb-1 mr-1 inline-flex rounded-md bg-indigo-50 px-2 py-1 text-[11px] font-medium text-indigo-700 hover:bg-indigo-100'; link.textContent = label(field); evidenceCell.append(link); }}); if (!evidenceCell.childNodes.length) add(evidenceCell, 'span', 'text-xs text-slate-400', 'No supported link'); tr.append(evidenceCell); table.append(tr); }}); document.getElementById('table-count').textContent = `${{filtered.length}} matching apps · ${{visible.length}} shown`; document.getElementById('page-label').textContent = `Page ${{currentPage}} of ${{pageCount}}`; document.getElementById('page-prev').disabled = currentPage === 1; document.getElementById('page-next').disabled = currentPage === pageCount; }}
  document.getElementById('search').addEventListener('input', () => {{ currentPage = 1; renderTable(); }}); categoryFilter.addEventListener('change', () => {{ currentPage = 1; renderTable(); }}); document.getElementById('page-prev').addEventListener('click', () => {{ currentPage = Math.max(1, currentPage - 1); renderTable(); }}); document.getElementById('page-next').addEventListener('click', () => {{ currentPage += 1; renderTable(); }}); renderTable();
  </script>
</body>
</html>'''
